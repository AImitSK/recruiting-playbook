---
title: Form Builder & Custom Fields API
description: API-Endpunkte fuer Form Builder Konfiguration und benutzerdefinierte Formularfelder.
---

<Callout type="info" title="Pro Feature">
Alle Form Builder und Custom Fields Endpunkte erfordern eine aktive Pro-Lizenz.
</Callout>

## Form Builder Konfiguration

Der Form Builder verwendet einen Draft/Publish-Workflow. Aenderungen werden zuerst als Draft gespeichert und muessen veroeffentlicht werden, um im Frontend sichtbar zu sein.

### Draft-Konfiguration laden

<EndpointHeader method="GET" path="/wp-json/recruiting/v1/form-builder/config" />

**Query-Parameter:**

| Parameter | Typ | Beschreibung |
|-----------|-----|--------------|
| `include_fields` | boolean | Verfuegbare Felder einschliessen (Standard: true) |
...
<EndpointHeader method="PUT" path="/wp-json/recruiting/v1/form-builder/config" />
...
<EndpointHeader method="POST" path="/wp-json/recruiting/v1/form-builder/publish" />

Validiert die Konfiguration und veroeffentlicht sie. Ab diesem Moment wird die neue Konfiguration im Frontend verwendet.
...
{
  "success": true,
  "message": "Formular wurde veroeffentlicht.",
  "published_version": 6
}
```
...
| Code | Beschreibung |
|------|--------------|
| `missing_required_field` | Pflichtfeld fehlt in der Konfiguration |
| `no_steps` | Mindestens ein Step erforderlich |
| `no_finale` | Abschluss-Step fehlt |
| `invalid_step` | Step hat keine ID oder Titel |

### Aenderungen verwerfen

<EndpointHeader method="POST" path="/wp-json/recruiting/v1/form-builder/discard" />

Setzt den Draft auf die veroeffentlichte Version zurueck.
...
{
  "success": true,
  "message": "Aenderungen wurden verworfen."
}
```

### Veroeffentlichte Konfiguration (oeffentlich)

<EndpointHeader method="GET" path="/wp-json/recruiting/v1/form-builder/published" />

**Berechtigung:** Keine (oeffentlich fuer Frontend)

Gibt die veroeffentlichte Formular-Konfiguration fuer das Frontend zurueck.

### Aktive Felder abrufen

<EndpointHeader method="GET" path="/wp-json/recruiting/v1/form-builder/active-fields" />

Gibt alle sichtbaren Felder fuer die Bewerberdetail-Ansicht zurueck.
...
<EndpointHeader method="POST" path="/wp-json/recruiting/v1/form-builder/reset" />

Setzt die Formular-Konfiguration auf die Standard-Konfiguration zurueck.
...
{
  "success": true,
  "message": "Formular wurde auf Standard zurueckgesetzt."
}
```
...
<EndpointHeader method="PUT" path="/wp-json/recruiting/v1/form-templates/{id}" />

### Template loeschen

<EndpointHeader method="DELETE" path="/wp-json/recruiting/v1/form-templates/{id}" />

<Callout type="warning" title="Templates in Verwendung">
Templates die von Stellen verwendet werden, koennen nicht geloescht werden.
</Callout>
...
<EndpointHeader method="GET" path="/wp-json/recruiting/v1/form-templates/default" />
...
| `missing_required_field` | Pflichtfeld fehlt in Formular-Konfiguration |
| `template_in_use` | Template wird von Stellen verwendet |
