...
final class Plugin {
    ...
    public function showActionSchedulerMissingNotice() : void {
		...
		<div class="notice notice-error">
			<p>
				<strong><?php 
        esc_html_e( 'Recruiting Playbook:', 'recruiting-playbook' );
        ?></strong>
				<?php 
        printf( 
    ...
    public function showActionSchedulerBrokenNotice() : void {
        ...
        ?></strong>
				<?php 
        esc_html_e( 'Action Scheduler was loaded, but its functions are not available. Email queue may not be functional.', 'recruiting-playbook' );
        ?>
			</p>
		</div>
		<?php 
    ...
    private function registerWebhookHooks() : void {
        ...
        add_action(
            ...
            [$service, 'onApplicationCreated'],
            10,
            2
        );
        add_action(
            'recruiting_playbook_application_status_changed',
            [$service, 'onApplicationStatusChanged'],
            10,
            3
        );
    ...
    public function checkApiKeyRateLimit( $result, $server, $request ) {
        ...
        if ( !$rate_check['allowed'] ) {
            return new \WP_Error('rate_limit_exceeded', __( 'Rate limit exceeded. Please try again later.', 'recruiting-playbook' ), [
                'status'      => 429,
    ...
    public function registerRestRoutes() : void {
        ...
        // Stub-Endpoints für Premium-Features (Free-Version Fallbacks).
        $this->registerFreeVersionStubEndpoints();
    }

    /**
     * Stub-Endpoints für Premium-Features registrieren
     *
     * Verhindert 500-Errors wenn React-Komponenten Premium-Endpoints aufrufen.
     * Gibt leere Arrays/Defaults zurück statt zu crashen.
     */
    private function registerFreeVersionStubEndpoints() : void {
        // Form-Builder active-fields (für Bewerbungsformular).
        register_rest_route(
            'recruiting/v1',
            '/form-builder/active-fields',
            [
                'methods'             => 'GET',
                'callback'            => function() {
        ...
        // Application Timeline/Activity (Pro-Feature).
        register_rest_route(
            'recruiting/v1',
        ...
        // Application Email Log (Pro-Feature).
        register_rest_route(
            'recruiting/v1',
    ...
    public function enqueueFrontendAssets() : void {
        ...
        if ( file_exists( $alpine_file ) ) {
            wp_enqueue_script(
                ...
                $alpine_deps,
                // Abhängigkeit: Komponenten-Scripts müssen zuerst laden
                '3.14.3',
                true
            );
        }
    ...
    public function enqueueAdminAssets( string $hook ) : void {
        ...
        $is_plugin_page = str_starts_with( $hook, 'toplevel_page_recruiting' ) || str_starts_with( $hook, 'recruiting_page_' ) || str_starts_with( $hook, 'admin_page_rp-' ) || 'job_listing' === get_post_type();
        if ( !$is_plugin_page ) {
            return;
        }
        // Admin CSS.
