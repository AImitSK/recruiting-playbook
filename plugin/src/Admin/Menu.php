...
class Menu {
    ...
    public function handleEarlyActions() : void {
        ...
        if ( 'set_status' === $action && !empty( $_GET['status'] ) ) {
            // Nonce kann mit oder ohne ID sein (React-UI verwendet generischen Nonce).
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $nonce = isset( $_GET['_wpnonce'] ) ? sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ) : '';
            if ( ! wp_verify_nonce( $nonce, 'rp_set_status_' ) && ! wp_verify_nonce( $nonce, 'rp_set_status_' . $id ) ) {
                wp_die( esc_html__( 'Security check failed.', 'recruiting-playbook' ) );
            }
    ...
    private function handleBulkActions() : void {
        ...
        switch ( $action ) {
            ...
            case 'bulk_email':
                return;
                // Silently ignore in Free version.
