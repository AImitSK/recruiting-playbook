...
class Menu {
    ...
    public function register() : void {
        ...
        // Kanban-Board (Pro-Feature) - nur wenn verfügbar.
        ...
        // Talent-Pool (Pro-Feature) - nur wenn verfügbar.
        ...
        // E-Mail-Vorlagen (Pro-Feature) - nur wenn verfügbar.
        if ( function_exists( 'rp_can' ) && rp_can( 'email_templates' ) ) {
            $email_callback = ( $this->email_settings_page ? [$this->email_settings_page, 'render'] : [$this, 'renderEmailTemplatesPlaceholder'] );
            add_submenu_page(
                'recruiting-playbook',
    ...
    private function handleBulkActions() : void {
        ...
        switch ( $action ) {
            ...
            case 'bulk_email':
                ...
                set_transient( 'rp_bulk_email_ids_' . $current_user->ID, $ids, 300 );
                // 5 Minuten gültig.
    ...
    private function handleBackupImport() : void {
        ...
        if ( empty( $_FILES['backup_file'] ) || empty( $_FILES['backup_file']['tmp_name'] ) ) {
            set_transient( 'rp_import_result', [
    ...
    public function renderBulkEmail() : void {
        ...
        wp_die( esc_html__( 'Bulk email requires Pro.', 'recruiting-playbook' ), esc_html__( 'Pro feature required', 'recruiting-playbook' ), [
            'response' => 403,
        ] );
    }
    ...
    private function renderBulkEmailPro() : void {
        ...
        if ( empty( $application_ids ) || !is_array( $application_ids ) ) {
			...
			<div class="wrap">
				<h1><?php 
            esc_html_e( 'Bulk Email', 'recruiting-playbook' );
            ?></h1>
				<div class="notice notice-error">
					<p><?php 
            esc_html_e( 'No applications selected. Please select applications from the list.', 'recruiting-playbook' );
            ?></p>
				</div>
				<p>
					<a href="<?php 
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $templates = ( $wpdb->get_results( "SELECT id, name, subject FROM {$templates_table} WHERE is_active = 1 ORDER BY name ASC", ARRAY_A ) ?: [] );
        ...
        $recipients = ( $wpdb->get_results( $wpdb->prepare( "SELECT a.id, c.first_name, c.last_name, c.email\n\t\t\t\t FROM {$applications_table} a\n\t\t\t\t LEFT JOIN {$candidates_table} c ON a.candidate_id = c.id\n\t\t\t\t WHERE a.id IN ({$placeholders})", ...$application_ids ), ARRAY_A ) ?: [] );
		...
		<div class="wrap">
			<h1><?php 
        esc_html_e( 'Send Bulk Email', 'recruiting-playbook' );
        ?></h1>
        ...
        foreach ( $recipients as $recipient ) {
            ?>
							<tr>
								<td><?php 
            echo esc_html( trim( $recipient['first_name'] . ' ' . $recipient['last_name'] ) );
            ?></td>
								<td><?php 
            echo esc_html( $recipient['email'] );
            ?></td>
							</tr>
						<?php 
        }
        ...
        if ( empty( $templates ) ) {
            ?>
					<div class="notice notice-warning inline">
						<p>
							<?php 
            esc_html_e( 'No email templates available.', 'recruiting-playbook' );
            ?>
							<a href="<?php 
            echo esc_url( admin_url( 'admin.php?page=rp-email-templates' ) );
            ?>">
								<?php 
            esc_html_e( 'Create templates', 'recruiting-playbook' );
            ?>
							</a>
						</p>
					</div>
            ...
            ?>
					<form method="post">
						<?php 
            wp_nonce_field( 'rp_bulk_email' );
            ?>

						<table class="form-table">
							<tr>
								<th scope="row">
									<label for="template_id"><?php 
            esc_html_e( 'Email Template', 'recruiting-playbook' );
            ?></label>
								</th>
								<td>
									<select name="template_id" id="template_id" class="regular-text" required>
										<option value=""><?php 
            esc_html_e( '— Select template —', 'recruiting-playbook' );
            ?></option>
										<?php 
            foreach ( $templates as $template ) {
                ...
                ?>
												(<?php 
                echo esc_html( $template['subject'] );
                ?>)
											</option>
										<?php 
            }
            ...
            ?>" target="_blank">
											<?php 
            esc_html_e( 'Manage templates', 'recruiting-playbook' );
            ?>
										</a>
									</p>
								</td>
						...
						<div class="notice notice-warning inline" style="margin: 15px 0;">
							<p>
								<strong><?php 
            esc_html_e( 'Warning:', 'recruiting-playbook' );
            ?></strong>
								<?php 
            esc_html_e( 'The emails will be sent immediately to all selected recipients.', 'recruiting-playbook' );
            ?>
							</p>
						</div>
            ...
            ?>" class="button">
								<?php 
            esc_html_e( 'Cancel', 'recruiting-playbook' );
            ?>
							</a>
						</p>
					</form>
